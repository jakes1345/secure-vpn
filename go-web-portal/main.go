...
import (
	...
	"secure-vpn/web-portal/services"
...
// TestEmailHandler handles requests to send a test email.
func TestEmailHandler(c *gin.Context) {
	emailService := services.NewFallbackEmailService()
	
	// Use a hardcoded test email for now
	to := "test@example.com"
	subject := "Go Web Portal Test Email"
	body := "<h1>Hello from Go!</h1><p>This email was sent using the new Go email service with fallback logic.</p>"

	err := emailService.SendEmail(to, subject, body)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("Failed to send email: %v", err.Error())})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": fmt.Sprintf("Test email successfully sent to %s", to)})
}
...
func SetupRouter() *gin.Engine {
	...
	// Simple API group
		api := router.Group("/api/v1")
		{
			...
			api.POST("/test-email", TestEmailHandler)
