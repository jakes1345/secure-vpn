#!/bin/bash
# Fix and rebuild .deb package properly
...
if [ -f installers/phazevpn-client_1.0.0_amd64.deb ]; then
    ...
    dpkg-deb -c installers/phazevpn-client_1.0.0_amd64.deb | head -10
else
    echo "❌ Build failed"
    exit 1
fi
"""
    ...
    print("   wget https://phazevpn.duckdns.org/download/client/linux -O phazevpn-client.deb")
    print("   sudo dpkg -i phazevpn-client.deb")
