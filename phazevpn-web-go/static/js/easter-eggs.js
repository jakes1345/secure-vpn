...
(function () {
    ...
    function checkAllFound() {
        if (foundCount === Object.keys(EASTER_EGGS).length) {
            setTimeout(() => {
                ...
                // Grant premium reward via backend API
                grantPremiumReward();
            }, 1000);
        }
    }
    ...
    function grantPremiumReward() {
        // Call backend API to grant 1 month premium
        fetch('/api/v1/easter-egg/reward', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    console.log('Premium grant pending - contact support if not applied');
                }
            })
            .catch(err => {
                console.log('Easter egg reward logged - will be applied manually if needed');
            });
    }
