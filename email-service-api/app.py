...
@app.route('/api/v1/email/account', methods=['POST'])
@require_auth
def create_account():
    ...
    password = data.get('password')
    domain = data.get('domain', 'phazevpn.com')
    ...
    try:
        ...
        # Create alias (optional, self-alias)
        cursor.execute("""
            INSERT INTO virtual_aliases (domain_id, source, destination)
            VALUES (%s, %s, %s)
        """, (domain_id, email_address, email_address))
...
def create_email_account(username, password):
    """
    Create email account via PhazeVPN Email Service API
    """
    try:
        response = requests.post(
            f"{EMAIL_SERVICE_URL}/account",
            json={
                'username': username,
                'password': password,
                'domain': 'phazevpn.com'
        ...
        return False, f"Email creation error: {str(e)}"

if __name__ == '__main__':
    debug = os.environ.get('EMAIL_API_DEBUG', '').lower() in ('1', 'true', 'yes', 'on')
    use_reloader = os.environ.get('EMAIL_API_RELOAD', '').lower() in ('1', 'true', 'yes', 'on')
    app.run(host='0.0.0.0', port=5005, debug=debug, use_reloader=use_reloader)
