...
func (s *PhazeVPNServer) handleHandshake(pkt *protocol.Packet, addr *net.UDPAddr) {
	// Check abuse prevention FIRST (before allowing connection)
	...
	// Assign client IP
	clientIP := s.ipPool.AssignIP(sessionID)
	...
	// Add route for this client IP
	s.routingTable.AddRoute(clientIP, sessionID)

	// Send handshake response with assigned IP
	response := protocol.NewHandshakePacket(sessionID, []byte(clientIP))
	...
	// Record connection in abuse prevention
	s.abusePrevention.RecordConnection(clientIP)

	log.Printf("✅ New session established: %d -> %s (%s)", sessionID, clientIP, addr)
}
...
func (s *PhazeVPNServer) handleData(pkt *protocol.Packet, addr *net.UDPAddr) {
	...
	s.abusePrevention.RecordTraffic(clientIP, int64(len(plaintext)))
}
...
// Helper functions
func getServerIP(network string) (string, error) {
